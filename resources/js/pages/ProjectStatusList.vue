          ...
          <v-flex xs12>
            <div>
              <v-toolbar flat color="white">
                <v-toolbar-title>PJステータス管理</v-toolbar-title>
                      ...
                      <v-container grid-list-md>
                        <v-form v-model="valid" ref="form">
                          <v-layout wrap>
                            <v-flex xs5 sm5 md5>
                              <v-text-field
                                v-model="projectStatusItem.name"
...
export default {
  ...
  data() {
    return {
      ...
      valid: false,
      projectStatuses: [],
      isProjectStatusExist: false,
      projectStatusItem: {},
      projectStatusItemDefault: {
        id: null,
        name: null,
        is_deleted: null
      }
  ...
  computed: {
    formTitle() {
      return this.isProjectStatusExist
        ? "PJステータス編集"
        : "PJステータス作成";
    }
  },
  methods: {
    /** 初期化 */
    initialize() {
      this.projectStatusItem = Object.assign({}, this.projectStatusItemDefault);
    },
    ...
    /** PJステータスデータ登録 */
    async store() {
      const response = await axios.post(`/api/projectstatus/store`, {
        projectStatus: this.projectStatusItem
      });
    ...
    /** PJステータスデータ編集 */
    async edit() {
      const response = await axios.post(`/api/projectstatus/edit`, {
        projectStatus: this.projectStatusItem
      });
    ...
    /** PJステータスデータ削除 */
    async delete() {
      const response = await axios.post(`/api/projectstatus/delete`, {
        projectStatusId: this.projectStatusItem.id
      });
    ...
    async save() {
      if (this.$refs.form.validate()) {
        // データ登録・編集
        this.isProjectStatusExist ? await this.edit() : await this.store();
        // モーダルクローズ
        this.close();
        // PJステータス一覧取得
        this.fetchProjectStatuses();
    ...
    close() {
      ...
      setTimeout(() => {
        this.projectStatusItem = Object.assign(
          {},
          this.projectStatusItemDefault
        );
        this.isProjectStatusExist = false;
      }, 300);
    }
  },
  watch: {
